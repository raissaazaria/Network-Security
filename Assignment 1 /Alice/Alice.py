...
while True:
    ...
    while True:
        ...
        # Separate the message from its hash
        msg, h = decrypted_msg[:-20], decrypted_msg[-20:]# Split decrypted data into message and hash
        h_prime = hashlib.sha1(ssk + msg).digest()

        # Verify if the received hash matches the computed hash
        if h == h_prime:
            if msg.decode() == "exit":
